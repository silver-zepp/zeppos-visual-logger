/** @about Visual Logger 1.5.0 @min_zeppos 2.0 @author: Silver, Zepp Health. @license: MIT */
...
let repeats_arr = new Array(DEFAULT_LINE_COUNT).fill(0);
...
export default class VisLog {
  ...
  // AppSide + AppSettings logger
  #appsettings_preffix = "AppSettings";
  #appside_preffix = "AppSide";
  #replay_preffix = "Replay";
  #appside_init_timeout = 20000; // 20s
   ...
   */
  refresh() {
    if (!this.#viewContainerExists()) return;
  ...
  updateSettings(settings) {
    ...
    if (typeof settings.background_enabled === "boolean") {
      ...
      if (this.#viewContainerExists() && this.#background_widget) {
        this.#background_widget.setProperty(
          hmUI.prop.VISIBLE,
  ...
  initSideRelay(vm, callback, options = {}) {
    ...
    const {
      method = 'INIT_VISLOG',
      timeout = this.#appside_init_timeout,
      retries = 2
    } = options;
    ...
    const try2init = () => {
      ...
      if (vm.request === undefined) {
        this.error("Vis for AppSide & AppSettings requires ZML to work!");
        return;
      }
      ...
      vm.request({ method })
        .then((response) => {
          ...
          if (callback) callback(null, { response, elapsed_ms, attempts: attempt });
        })
        .catch((error) => {
          if (attempt < retries) {
            setTimeout(try2init, 1000);
          } else {
            const err = new Error(`App-side service init failed after ${retries} attempts: ${error.message}`);
            this.error(`[${this.#appside_preffix}] ${err.message}`);
    ...
    const to_id = setTimeout(() => {
      const elapsed_ms = Date.now() - start_time;
      const to_err = new Error('App-side service initialization timeout');
      this.error(`[${this.#appside_preffix}] ${to_err.message}`);
      if (callback) callback(to_err, { elapsed_ms, attempts: attempt });
    }, timeout);

    const og_cb = callback;
    callback = (error, result) => {
      clearTimeout(to_id);
      if (og_cb) og_cb(error, result);
    };
  ...
  handleSideServiceCall(data) {
    if (!data || data.type !== 'VIS_DBG_LOG') {
      return false;
    }

    this.#displaySideServiceLog(data.data);
    return true;
  }

  #displaySideServiceLog(log_entry) {
    if (!log_entry || log_entry === null) {
      this.warn("Received null log entry from settings");
      return;
    }

    // TODO: optionally show timestamp maybe?
    const { level, message, filename, source, is_replay, timestamp } = log_entry;

    if (!level || !message) {
      this.warn("Received incomplete log entry:", JSON.stringify(log_entry));
      return;
    }
    ...
    if (is_replay) {
      prefix = this.#replay_preffix;
    } else if (source === 'AppSide') { // hardcode OK
      prefix = this.#appside_preffix;
    } else {
      prefix = this.#appsettings_preffix; // AppSettings
    }

    const msg = `[${filename || prefix}] ${message}`;
  ...
  #removeOldestMessage() {
    ...
    if (num_stored_messages > 0) {
      ...
      if (this.#visual_log_enabled && this.#viewContainerExists()) {
        this.#renderText();
      }
  ...
  #viewContainerExists() {
    ...
    try {
      ...
      if (this.#view_container && typeof this.#view_container.setProperty === 'function') {
        return true;
      }
  ...
  #shouldEnableVis() {
    return this.#viewContainerExists() && this.#visual_log_enabled;
  }
 ...
 * 1.5.0
 * - @add support for AppService logs
 * - @add support for AppSettings logs
 * - @add support for logging from app.js (console only)
 * - @add minification
 */